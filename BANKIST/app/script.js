...
// Load any custom accounts registered on the landing page
  ...
  movs.forEach(function (mov, i) {
    ...
    const html = `
      ...
      i + 1
        ...
        <div class="movements__date">Recently</div>
...
btnLogin.addEventListener('click', function (e) {
  ...
  currentAccount = accounts.find(
    acc => acc.username.toLowerCase() === inputLoginUsername.value.trim().toLowerCase()
  );
  ...
  if (currentAccount && currentAccount.pin === Number(inputLoginPin.value)) {
    ...
    alert('Incorrect user ID or PIN! Try js (PIN: 1111) or jd (PIN: 2222).');
  }
});
...
btnLoan.addEventListener('click', function (e) {
  ...
  if (amount > 0 && currentAccount.movements.some(mov => mov >= amount * 0.1)) {
    ...
    alert('Loan denied: You need at least one deposit worth 10% of requested loan amount.');
  }
...
// Auto login if directed from account creation
const activeUsername = localStorage.getItem('bankist_active_user');
if (activeUsername) {
  const autoAcc = accounts.find(
    acc => acc.username.toLowerCase() === activeUsername.toLowerCase()
  );
  if (autoAcc) {
    currentAccount = autoAcc;
